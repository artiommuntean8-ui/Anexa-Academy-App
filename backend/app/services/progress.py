...
logger = logging.getLogger("app.services.progress")
...
def _get_student_lessons(db: Session, student_id: int) -> tuple[list[Course], list[Assignment], dict[int, Grade]]:
    try:
        ...
        logger.error(f"Error getting student lessons for student {student_id}: {e}")
        return [], [], {}
...
def build_student_progress(db: Session, student: Student, include_lessons: bool = False) -> dict:
    try:
        ...
        logger.error(f"Error building student progress for student {student.id}: {e}")
        return {
            "student": student,
